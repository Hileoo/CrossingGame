...
/**
 * The Map1 class is used to sets up basic background, operation and initiates of map1
 ...
 *
 */
public class Map1 implements MouseListener
{
	...
	private final int xStart = 12, yStart = 4;
	private final int xFinish = 0, yFinish = 4;
	private int xPos = 12;//The initial x-location of man
	private int yPos = 4;//The initial y-location of man
	...
	//Map1 background information, used for implementing the action
	private String mapArray1[][] = {
			{"water","water","water","water","stump","water","water","water","water"},
			{"water","water","water","water","water","water","water","water","water"},
			{"stump","water","water","water","water","water","water","water","water"},
			{"water","water","water","water","water","water","water","water","water"},
			{"water","water","water","water","stump","water","water","water","water"},
			{"water","water","water","water","water","water","water","water","water"},
			{"stump","water","stump","plank","plank","plank","stump","water","water"},
			{"water","water","water","water","water","water","water","water","water"},
			{"stump","water","water","water","stump","water","water","water","water"},
			{"plank","water","water","water","plank","water","water","water","water"},
			{"stump","water","water","water","plank","water","water","water","water"},
			{"water","water","water","water","plank","water","water","water","water"},
			{"water","water","water","water","stump","water","water","water","water"},};
	
    /**
     * Constructor for class Map1 
     */
    public Map1()
    {
        ...
        this.map1Background(frame,panel,button);
        
        this.addButton(panel, button);
        this.mouseListener();
        
        frame.setTitle("The Perilous Plank Puzzle Conundrum - Level1");
    ...
    public void setWindow()
    {
        ...
        score.readAllScore("Score/score1.txt");
    ...
    /**
     * map1Background method provide the specific image background of map1
     * Replace some button in "basicBackground" method
     * @param frame  the frame to add panel in map1
     * @param panel  the panel to add button in map1
     * @param button  the button array in map1, added in the main panel, set as ImageIcon
     */
    public void map1Background(JFrame frame,JPanel panel,JButton button[][])
    {
    	button[12][4] = setImageIcon(image.getStump2Man(), new JButton());
        button[0][4] = setImageIcon(image.getStump3(), new JButton());
        button[10][0] = setImageIcon(image.getStump1(), new JButton());
        button[8][0] = setImageIcon(image.getStump1(), new JButton());
        button[6][0] = setImageIcon(image.getStump1(), new JButton());
        button[2][0] = setImageIcon(image.getStump1(), new JButton());
        button[6][2] = setImageIcon(image.getStump1(), new JButton());
        button[6][6] = setImageIcon(image.getStump1(), new JButton());
        button[4][4] = setImageIcon(image.getStump1(), new JButton());
        button[8][4] = setImageIcon(image.getStump1(), new JButton());
        button[2][4] = setImageIcon(image.getWater3(), new JButton());
        button[8][2] = setImageIcon(image.getWater3(), new JButton());
        button[4][0] = setImageIcon(image.getWater4(), new JButton());
        button[7][5] = setImageIcon(image.getWater4(), new JButton());
        button[9][4] = setImageIcon(image.getPlank2(), new JButton());
        button[10][4] = setImageIcon(image.getPlank2(), new JButton());
        button[11][4] = setImageIcon(image.getPlank2(), new JButton());
        button[9][0] = setImageIcon(image.getPlank2(), new JButton());
        button[6][3] = setImageIcon(image.getPlank1(), new JButton());
        button[6][4] = setImageIcon(image.getPlank1(), new JButton());
        button[6][5] = setImageIcon(image.getPlank1(), new JButton());       
    }
	 ...
	 * Include promote to next level function: when man reach the designed goal finishing position in map1, start the next level map2
	...
	public void moveMan(int xMan, int yMan, int xClick, int yClick)
	{
		...
		if((xMan == xClick)&&(Math.abs(yMan-yClick) > 0))
		{
			...
			if(yMan < yClick)
			{
				...
				for(int i=1;i<=Math.abs(yMan-yClick);i++)//fix i=0 to i=1
				{
					...
					if(mapArray1[xMan][yMan+i] != "stump" && mapArray1[xMan][yMan+i] != "plank")
					{
						canMove = false;
						break;
					}
		...
		if((yMan == yClick)&&(Math.abs(xMan-xClick) > 0))
		{
			...
			if(xMan < xClick)
			{
				...
				{
					//can not move when the path is neither on a stump nor on a plank
					if(mapArray1[xMan+i][yMan] != "stump" && mapArray1[xMan+i][yMan] != "plank")
					{
						canMove = false;
						break;
					}
				...
				if(canMove == true)
				{
					...
					else if(mapArray1[xMan][yMan] == "stump")
					{
						setImageIcon(image.getStump1(), button[xMan][yMan]);
					}
					else if(mapArray1[xMan][yMan] == "plank")
					...
					else if(mapArray1[xClick][yClick] == "stump")
					{
						setImageIcon(image.getStump1Man(), button[xClick][yClick]);
					}
					else if(mapArray1[xClick][yClick] == "plank")
			...
			else if(xMan > xClick)
			{
				...
				if(canMove == true)
				{
					...
					if(xClick == xFinish && yClick == yFinish)
					{
						...
						frame.dispose();//Close the window of map1
						Score score = new Score();
						score.writeScore(timerInfo.getText(), "Score/score1.txt");//record the time (score) in file "score1.txt"
						new Map2();//start the next map2		
					}
					else if(mapArray1[xClick][yClick] == "stump")
					{
						setImageIcon(image.getStump1Man(), button[xClick][yClick]);
	...
	/**
	 * The method to Pick Up and Carry a Plank in map1
	...
	public void carryPlank(int xMan, int yMan,int xClick, int yClick)
	{
		if(canCarryPlank = true);
		{
			...
			if((yMan == yClick) && Math.abs(xMan - xClick)>0 && Math.abs(xMan-xClick)<7)
			{
				...
				{
					if(mapArray1[xMan][yMan] == "stump" && mapArray1[xClick][yClick] == "water")
					{
						canCarryPlank = false;
					}
				...
				if(mapArray1[xMan][yMan] == "stump" && mapArray1[xClick][yClick] == "stump")
				{
				...
				if(mapArray1[xMan][yMan] == "stump" && mapArray1[xClick][yClick] == "plank")
				{
					...
					if(xMan > xClick)
					{
						...
						for(int i=1;i<=MAX_LENGTH;i++)
						{
							...
							else if(mapArray1[xMan-i][yMan] == "stump")
							{
								break;
							}
						}
			...
			if((xMan == xClick) && Math.abs(yMan - yClick)>0 && Math.abs(yMan-yClick)<7)
			{
				...
				if(mapArray1[xMan][yMan] == "stump" && mapArray1[xClick][yClick] == "stump")
				{
					...
					{						
						...
						{
							//If in the middle of man_stump and clicked stump has other stump, can not pick up
							if(mapArray1[xMan][yMan-i] == "stump")
							{
								canCarryPlank = false;
								break;
							}
							//If in the middle of man_stump and another stump is not all be plank, can not pick up
							if(mapArray1[xMan][yMan-i] != "plank")
							{
								canCarryPlank = false;
								break;
							}
				...
				if(mapArray1[xMan][yMan] == "stump" && mapArray1[xClick][yClick] == "plank")
				{
					...
					if(yMan > yClick)
					{
						...
						for(int i=1;i<=MAX_LENGTH;i++)
						{
							...
							else if(mapArray1[xMan][yMan-i] == "stump")
							{
								break;
							}
						}
				...
				if((yMan == yClick) && Math.abs(xMan - xClick)>0 && Math.abs(xMan-xClick)<7)
				{
					...
					if(mapArray1[xMan][yMan] == "stump" && mapArray1[xClick][yClick] == "stump")
					{
						...
						if(xMan > xClick)
						{
							for(int i=1;i<Math.abs(xMan - xClick);i++)
							{
								...
								if(mapArray1[xMan-i][yMan] != "water")
								{
									canSetPlank = false;
									break;
								}	
					...
					if(mapArray1[xMan][yMan] == "stump" && mapArray1[xClick][yClick] == "water")
					{
						...
						if(xMan > xClick)
						{
							...
							if(mapArray1[xMan-distance][yMan] != "stump")
							{
								canSetPlank = false;
							}
				...
				if((xMan == xClick) && Math.abs(yMan - yClick)>0 && Math.abs(yMan-yClick)<7)
				{
					...
					if(mapArray1[xMan][yMan] == "stump" && mapArray1[xClick][yClick] == "stump")
					{
						...
						if(yMan > yClick)
						{
							for(int i=1;i<Math.abs(yMan - yClick);i++)
							{
								...
								//If in the middle of man_stump and clicked stump is not all be water, can not place
								if(mapArray1[xMan][yMan-i] != "water")
								{
									canSetPlank = false;
									break;
								}	
					...
					if(mapArray1[xMan][yMan] == "stump" && mapArray1[xClick][yClick] == "water")
					{
						...
						if(yMan > yClick)
						{
							...
							//If the end is not another stump, can not place
							if(mapArray1[xMan][yMan-distance] != "stump")
							{
								canSetPlank = false;
							}
