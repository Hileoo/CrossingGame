...
/**
 * The Map2 class is used to sets up basic background, operation and initiates of map2
 ...
 *
 */
public class Map2 implements MouseListener
{
	...
	//Start information of map2
	private final int xStart = 12, yStart = 2;
	private final int xFinish = 0, yFinish = 2;
	...
	//Map2 background information, used for implementing the action
	private String mapArray2[][] = {
			{"water","water","stump","water","water","water","water","water","water"},
			{"water","water","water","water","water","water","water","water","water"},
			{"stump","water","water","water","water","water","stump","water","stump"},
			{"water","water","water","water","water","water","plank","water","water"},
			{"water","water","stump","water","water","water","plank","water","stump"},
			{"water","water","water","water","water","water","plank","water","water"},
			{"stump","water","water","water","stump","water","plank","water","stump"},
			{"plank","water","water","water","water","water","plank","water","plank"},
			{"plank","water","water","water","water","water","stump","water","plank"},
			{"plank","water","water","water","water","water","water","water","plank"},
			{"stump","water","stump","water","stump","water","water","water","stump"},
			{"water","water","plank","water","water","water","water","water","water"},
			{"water","water","stump","water","water","water","water","water","water"},};
	
    /**
     * Constructor for class Map2 
     */
    public Map2()
    {
        ...
        this.map2Background(frame,panel,button);
        
        this.addButton(panel, button);
        this.mouseListener();
        
        frame.setTitle("The Perilous Plank Puzzle Conundrum - Level2");
    ...
    public void setWindow()
    {
        ...
        score.readAllScore("Score/score2.txt");
    ...
    /**
     * map2Background method provide the specific image background of map2
     * Replace some button in "basicBackground" method
     * @param frame  the frame to add panel in map2
     * @param panel  the panel to add button in map2
     * @param button  the button array in map2, added in the main panel, set as ImageIcon
     */
    public void map2Background(JFrame frame,JPanel panel,JButton button[][])
    {
    	button[12][2] = setImageIcon(image.getStump2Man(), new JButton());
        button[0][2] = setImageIcon(image.getStump3(), new JButton());
        button[2][0] = setImageIcon(image.getStump1(), new JButton());
        button[6][0] = setImageIcon(image.getStump1(), new JButton());
        button[10][0] = setImageIcon(image.getStump1(), new JButton());
        button[4][2] = setImageIcon(image.getStump1(), new JButton());
        button[10][2] = setImageIcon(image.getStump1(), new JButton());
        button[6][4] = setImageIcon(image.getStump1(), new JButton());
        button[10][4] = setImageIcon(image.getStump1(), new JButton());
        button[2][6] = setImageIcon(image.getStump1(), new JButton());
        button[8][6] = setImageIcon(image.getStump1(), new JButton());
        button[2][8] = setImageIcon(image.getStump1(), new JButton());
        button[4][8] = setImageIcon(image.getStump1(), new JButton());
        button[6][8] = setImageIcon(image.getStump1(), new JButton());
        button[10][8] = setImageIcon(image.getStump1(), new JButton());
        button[5][8] = setImageIcon(image.getWater3(), new JButton());
        button[4][0] = setImageIcon(image.getWater4(), new JButton());
        button[2][2] = setImageIcon(image.getWater4(), new JButton());
        button[8][2] = setImageIcon(image.getWater4(), new JButton());
        button[6][5] = setImageIcon(image.getWater4(), new JButton());
        button[4][7] = setImageIcon(image.getWater4(), new JButton());
        button[6][2] = setImageIcon(image.getWater2(), new JButton());
        button[5][4] = setImageIcon(image.getWater2(), new JButton());
        button[10][6] = setImageIcon(image.getWater2(), new JButton());
        button[7][0] = setImageIcon(image.getPlank2(), new JButton());
        button[8][0] = setImageIcon(image.getPlank2(), new JButton());
        button[9][0] = setImageIcon(image.getPlank2(), new JButton());
        button[11][2] = setImageIcon(image.getPlank2(), new JButton());
        button[3][6] = setImageIcon(image.getPlank2(), new JButton());
        button[4][6] = setImageIcon(image.getPlank2(), new JButton());
        button[5][6] = setImageIcon(image.getPlank2(), new JButton());
        button[6][6] = setImageIcon(image.getPlank2(), new JButton());
        button[7][6] = setImageIcon(image.getPlank2(), new JButton());
        button[7][8] = setImageIcon(image.getPlank2(), new JButton());
        button[8][8] = setImageIcon(image.getPlank2(), new JButton());
	...
	/**
	 * The method to move the man when LEFT CLICK in map1
	 * Include promote to next level function: when man reach the designed goal finishing position of map2, start the next level map3
	...
	public void moveMan(int xMan, int yMan, int xClick, int yClick)
	{
		...
		if((xMan == xClick)&&(Math.abs(yMan-yClick) > 0))
		{
			...
			if(yMan < yClick)
			{
				...
				for(int i=1;i<=Math.abs(yMan-yClick);i++)//fix i=0 to i=1
				{
					...
					if(mapArray2[xMan][yMan+i] != "stump" && mapArray2[xMan][yMan+i] != "plank")
					{
						canMove = false;
						break;
					}
			...
			else if(yMan > yClick)
			{
				...
				{
					//can not move when the path neither on a stump nor on a plank
					if(mapArray2[xMan][yMan-i] != "stump" && mapArray2[xMan][yMan-i] != "plank")
					{
						canMove = false;
						break;
		...
		if((yMan == yClick)&&(Math.abs(xMan-xClick) > 0))
		{
			...
			if(xMan < xClick)
			{
				//The minimum distance is 0, and maximum distance is |yMan-yClick|
				for(int i=1;i<=Math.abs(xMan-xClick);i++)
				{
					//can not move when the path is neither on a stump nor on a plank
					if(mapArray2[xMan+i][yMan] != "stump" && mapArray2[xMan+i][yMan] != "plank")
					{
						canMove = false;
						break;
					}
				...
				if(canMove == true)
				{
					...
					else if(mapArray2[xMan][yMan] == "stump")
					{
						setImageIcon(image.getStump1(), button[xMan][yMan]);
					}			
					else if(mapArray2[xMan][yMan] == "plank")
					...
					else if(mapArray2[xClick][yClick] == "stump")
					{
						setImageIcon(image.getStump1Man(), button[xClick][yClick]);
					}	
					else if(mapArray2[xClick][yClick] == "plank")
			...
			else if(xMan > xClick)
			{
				...
				if(canMove == true)
				{
					...
					if(xClick == xFinish && yClick == yFinish)
					{
						...
						frame.dispose();//Close the window of map2
						Score score = new Score();
						score.writeScore(timerInfo.getText(), "Score/score2.txt");//record the time (score) in file "score2.txt"
						new Map3();//start the next map3			
					}
					else if(mapArray2[xClick][yClick] == "stump")
					{
						setImageIcon(image.getStump1Man(), button[xClick][yClick]);
	...
	/**
	 * The method to Pick Up and Carry a Plank in map2
	...
	public void carryPlank(int xMan, int yMan,int xClick, int yClick)
	{
		if(canCarryPlank = true);
		{
			...
			if((yMan == yClick) && Math.abs(xMan - xClick)>0 && Math.abs(xMan-xClick)<7)
			{
				...
				{
					if(mapArray2[xMan][yMan] == "stump" && mapArray2[xClick][yClick] == "water")
					{
						canCarryPlank = false;
					}
				...
				if(mapArray2[xMan][yMan] == "stump" && mapArray2[xClick][yClick] == "stump")
				{
				...
				if(mapArray2[xMan][yMan] == "stump" && mapArray2[xClick][yClick] == "plank")
				{
					...
					if(xMan > xClick)
					{
						...
						for(int i=1;i<=MAX_LENGTH;i++)
						{
							...
							else if(mapArray2[xMan-i][yMan] == "stump")
							{
								break;
							}
						}
					...
					if(xMan < xClick)
					{
						...
						for(int i=1;i<=length;i++)
						{
							//If in the middle of man_stump and another stump has other stump, can not pick up/中间有stump，不能取木板
							if(mapArray2[xMan+i][yMan] == "stump")
							{
								canCarryPlank = false;
								break;
			...
			if((xMan == xClick) && Math.abs(yMan - yClick)>0 && Math.abs(yMan-yClick)<7)
			{
				...
				if(mapArray2[xMan][yMan] == "stump" && mapArray2[xClick][yClick] == "stump")
				{
					...
					{						
						...
						{
							//If in the middle of man_stump and clicked stump has other stump, can not pick up
							if(mapArray2[xMan][yMan-i] == "stump")
							{
								canCarryPlank = false;
								break;
							}
							//If in the middle of man_stump and another stump is not all be plank, can not pick up
							if(mapArray2[xMan][yMan-i] != "plank")
							{
								canCarryPlank = false;
								break;
							}
				...
				if(mapArray2[xMan][yMan] == "stump" && mapArray2[xClick][yClick] == "plank")
				{
					...
					if(yMan > yClick)
					{
						...
						for(int i=1;i<=MAX_LENGTH;i++)
						{
							...
							else if(mapArray2[xMan][yMan-i] == "stump")
							{
								break;
							}
						}
	...
	/**
	 * The method to place a plank in map2
				...
				if((yMan == yClick) && Math.abs(xMan - xClick)>0 && Math.abs(xMan-xClick)<7)
				{
					...
					if(mapArray2[xMan][yMan] == "stump" && mapArray2[xClick][yClick] == "stump")
					{
						...
						if(xMan > xClick)
						{
							for(int i=1;i<Math.abs(xMan - xClick);i++)
							{
								...
								if(mapArray2[xMan-i][yMan] != "water")
								{
									canSetPlank = false;
									break;
								}
					...
					if(mapArray2[xMan][yMan] == "stump" && mapArray2[xClick][yClick] == "water")
					{
						...
						if(xMan > xClick)
						{
							...
							if(mapArray2[xMan-distance][yMan] != "stump")
							{
								canSetPlank = false;
							}
				...
				if((xMan == xClick) && Math.abs(yMan - yClick)>0 && Math.abs(yMan-yClick)<7)
				{
					...
					if(mapArray2[xMan][yMan] == "stump" && mapArray2[xClick][yClick] == "stump")
					{
						...
						if(yMan > yClick)
						{
							for(int i=1;i<Math.abs(yMan - yClick);i++)
							{
								...
								//If in the middle of man_stump and clicked stump is not all be water, can not place
								if(mapArray2[xMan][yMan-i] != "water")
								{
									canSetPlank = false;
									break;
								}	
							...
							//If the distance of clicked stump and man stump is not equal to carried plank's length, can not place//如果点击的地方与人之间的距离不等于木板长度
					...
					if(mapArray2[xMan][yMan] == "stump" && mapArray2[xClick][yClick] == "water")
					{
						...
						if(yMan > yClick)
						{
							...
							//If the end is not another stump, can not place
							if(mapArray2[xMan][yMan-distance] != "stump")
							{
								canSetPlank = false;
							}
							//Except the end point, Test the middle elements
